...
GameData *setupGameData() {
  ...
  if (fp != NULL) {
		...
		free(jsonStr);
		int x = cJSON_GetObjectItem(cJSON_GetObjectItem(cJSON_GetObjectItem(json, "player"), "pos"), "x")->valueint;
		int y = cJSON_GetObjectItem(cJSON_GetObjectItem(cJSON_GetObjectItem(json, "player"), "pos"), "y")->valueint;
